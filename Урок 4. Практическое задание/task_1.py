...
def eratosphene(n):
    ...
    for number in numbers:
        if number != 0:
            for candidate in range(2 * number, n + 1, number):
                numbers[candidate - 2] = 0
    # return list(lambda x: x !=0 for x in numbers) - этот вывод для списка без нулей
    return numbers
...
"""
По итогу тестов на входных данных в 15_000_000
Время работы чистым кодом 13.32
Время работы функцией 9.979
Разница в эффективности составляет -33.53% в пользу работы функцией
Так как функция работает в локальной области данных, алгоритм работает быстрее 
"""